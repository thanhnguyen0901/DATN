        ...
        db.transaction(tx => {
          tx.executeSql(query, [], (tx, results) => {
            ...
            if (soDong == 0) {
              ...
              let maTN = "tn";
              db.transaction(tx => {
                tx.executeSql("SELECT ma_thu_nhap FROM thunhap WHERE ma_thu_nhap like (SELECT MAX(ma_thu_nhap) FROM thunhap)", [], (tx, results) => {
                  data = results.rows.item(0).ma_thu_nhap;
                  soHienTai = parseInt(data.slice(2, 6), 10) + 1;
                  let str = "" + soHienTai;
                  let pad = "0000";
                  maTN = maTN + pad.substring(0, pad.length - str.length) + str;
                  resolve(maTN);
                });
              });
            }
    ...
    render(){
      ...
      return(
        <Container>
          ...
          <Card>
            ...
            <CardItem>
              <InputGroup borderType="underline" >
                  ...
                  <Input  placeholder="0" style={{fontSize:20, color:'green', textAlign:'right', fontWeight:'bold'}} 
          ...
          <Card>
            <CardItem button onPress={() => navigation.navigate('ChonHangMucThu', { returnDataHangMuc: this.returnDataHangMuc.bind(this) })}
            ...
            <CardItem style={{borderColor:'grey', borderBottomWidth: 0.7, height: 50}}>
              <Item>
                ...
                <Input placeholder='Mô tả' placeholderTextColor='#3a445c' style={{flex: 9, borderBottomWidth: 0.1, fontSize: 18, color: 'black', paddingLeft: 12}}/>
              </Item>
            </CardItem>
            ...
            <CardItem button onPress={() => navigation.navigate('ThuTuAi', { returnDataNguoiThu: this.returnDataNguoiThu.bind(this) })}>
              <Left style={{flex: 1}}>
